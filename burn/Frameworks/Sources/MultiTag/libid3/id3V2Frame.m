...
-(NSData *)getCompleteRawFrame
{
    ...
    if ((majorVersion == 4)||(majorVersion == 3))
    {
		...
		[completeFrame appendBytes:[frameID cStringUsingEncoding:NSASCIIStringEncoding] length:4];
    ...
    {
            [completeFrame appendBytes:[frameID cStringUsingEncoding:NSASCIIStringEncoding] length:3];
...
/*  V2.0-2.2 encoding
	...
	[frame appendData: [[Image objectForKey:@"Image"] representationUsingType:imageEncoding properties:@{}]];
...
{  //Splits the genre string into an array of string. Number are converted to strings base on the dictionary. if the dictionary is NULL the system used the inbuilt dictionary.
    ...
    const char * tempPointer = [Data bytes];
    int start = 0;
    int end = 0;
    int foundNull = 0;
    int foundBracket = 0;
